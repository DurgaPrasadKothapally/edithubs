...
interface LoginFormProps { error?: string; }
...
export function LoginForm({ error: urlError }: LoginFormProps) {
  ...
  const [error, setError] = useState(urlError === 'auth_failed' ? 'Authentication failed.' : '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isAdmin = data.user.email?.toLowerCase() === ADMIN.toLowerCase();
      toast.success(isAdmin ? 'Welcome back, Admin!' : 'Signed in!');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      <div className="text-center mb-6">
        ...
        <p className="text-text-muted text-sm mt-1">Admin accounts go directly to the dashboard.</p>
      </div>
      <div>
      ...
      <div className="text-center pt-2">
        <Link href="/" className="text-text-muted text-sm hover:text-accent transition-colors">Continue browsing without signing in</Link>
      </div>
    </form>
  );
}