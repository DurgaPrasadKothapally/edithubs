...
export function AdminTutorialActions({ tutorial }: { tutorial: Tutorial }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [deleting, setDeleting] = useState(false);

  const handleDelete = async () => {
    if (!confirm(`Delete "${tutorial.title}"? This cannot be undone.`)) return;
    setDeleting(true);
  ...
  return (
    <div className="flex items-center justify-end gap-2">
      <Link
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface border border-surface-border text-xs text-text-secondary hover:text-accent hover:border-accent/30 transition-all"
      >
        <Edit size={12} />
        Edit
      </Link>
          ...
          <>
            ...
            <div className="absolute right-0 top-full mt-1 w-44 glass-dark border border-surface-border rounded-xl shadow-glass z-20 overflow-hidden">
              ...
              <Link
                ...
                target="_blank"
                className="flex items-center gap-2.5 px-3.5 py-2.5 text-xs text-text-secondary hover:text-text-primary hover:bg-surface transition-colors"
